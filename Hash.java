...
* @author Alyssa Reyes
* 
* This class defines the Hash.java class and the methods needed to 
* insert, remove, and search for values in a hash table.
* 
...
public class Hash<T extends Comparable<T>> {
	 ...
	 * according to value passed in as a parameter. Inserts size empty Lists
	 * into the table. Sets numElements to 0
	...
	/**
	 * Returns the hash value in the Table for a given key by taking modulus
	 * of the hashCode value for that key and the size of the table
	 * 
	 * @param t the key
	...
	public int countBucket(int index) throws IndexOutOfBoundsException {
		if (index < 0 || index >= Table.size()) {
			throw new IndexOutOfBoundsException("countBucket(): Index " 
	 ...
	 * @return the index in the Table at which the element 
	...
	/**
	 * Inserts a new key in the Table calls the hash method to 
	 * determine placement
	 * 
	...
	public void remove(T t) {
		if(t != null) { 
			int bucket = hash(t);
			int foundIndex = Table.get(bucket).linearSearch(t);
			if(foundIndex != -1) {
				Table.get(bucket).advanceToIndex(foundIndex);
	...
	/**
	 * Prints all the keys at a specified bucket in the Table. Each 
	 * key displayed on its own line, with a blank line separating each 
	 * key Above the keys, prints the message "Printing bucket #<bucket>:"
	...
	/**
	 * Starting at the first bucket, and continuing in order until 
	 * the last bucket, concatenates all elements at all buckets 
	 * into one String
	 */
	@Override
